...
class SignatureIntegrityPatcher:
    ...
    def patch(self) -> int:
        self.log("[*] [SigIntegrity] Đang loại bỏ integrity checks...")
        patched = 0
        for filepath in get_all_smali_files(self.decompiled_path):
            if len(filepath) > 250: continue
            content = self._read_file(filepath)
            if 'MessageDigest' not in content and 'Signature' not in content: continue

            for match in REGEX_INTEGRITY_METHOD.finditer(content):
                full = match.group(0)
                name = match.group(1).split('(')[0]
                if any(kw in name.lower() for kw in ['integrity', 'verify', 'check', 'hash', 'digest']):
                    header = full.split('\n')[0]
        ...
        self.log(f"[*] [SigIntegrity] Đã vá {patched} file")
        return patched